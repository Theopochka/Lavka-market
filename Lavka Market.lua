...
local lmUrl = "https://bitbucket.org/update-lavka-market/lavka/raw/d2b0d76d9e491fe6ce022e896ba94dceda4ca952/Lavka%20Market.lua"
...
autologin =
    {
        ...
        spawn = "[{\"id\": 0, \"spawn\": \"Ïîñëåäíåå ìåñòî âûõîäà\" },{\"id\": 1, \"spawn\": \"Ñòàíäàðòíûé ñïàâí\" }]", 
        spawn_use = 0,
    },  
    autoreconnect = {
         onBanned = false,
...
sampAddChatMessage("[Lavka Market]: {FFFFFF} Ïðîâåðêà íàëè÷èÿ îáíîâëåíèé...", 0x21D518)
    ...
    if response and response ~= currentVersion then
    	sampAddChatMessage("[Lavka Market]: {FFFFFF} Ó âàñ íå àêòóàëüíàÿ âåðñèÿ! Äëÿ îáíîâëåíèÿ ïåðåéäèòå âî âêëàäêó Èíôî", 0x21D518)
    else
...
-- Òåïåðü ïåðåìåííûå äîñòóïíû íàïðÿìóþ èç ìàññèâîâ áåç ãëîáàëüíîé îáëàñòè âèäèìîñòè
-- Íàïðèìåð, variables[1].se, variables[1].i, boolVariables[1]
bnd = {
	imgui.new.int(0),
	new.char[100](),
	""
...
local method = {u8'×èïñû', u8'Ðûáà', u8'Îëåíèíà'}
...
function save_log(logtext, filename)
    ...
    -- Îòêðûâàåì ôàéë äëÿ äîáàâëåíèÿ íîâîãî ëîãà
    local file = io.open(file_path, "a")

    if file then
        -- Çàïèñûâàåì ëîã â ôàéë ñ äàòîé è âðåìåíåì
...
function sampev.onServerMessage(color, text)
    if text:find('^%[Ïîäñêàçêà%] {FFFFFF}Âû óñïåøíî àðåíäîâàëè ëàâêó äëÿ ïðîäàæè') or text:find('^%[Ïîäñêàçêà%] {FFFFFF}Âû óñïåøíî âûñòàâèëè ëàâêó äëÿ ïðîäàæè.ïîêóïêè òîâàðà.$') then
        save_log("Âû àðåíäîâàëè ëàâêó", logfile)
        msg('Äëÿ òîãî, ÷òîáû îòêðûòü ìåíþ ââåäèòå êîìàíäó: /cent')
...
SkupSellzxc[0] = not SkupSellzxc[0]
    ...
    elseif  (text:find('^%[Èíôîðìàöèÿ%] {FFFFFF}Âû îòêàçàëèñü îò àðåíäû ëàâêè') or text:find('^%[Èíôîðìàöèÿ%] {FFFFFF}Âû ñíÿëè ëàâêó') or text:find('^%[Èíôîðìàöèÿ%] {FFFFFF}Ó Âàñ çàêîí÷èëîñü âðåìÿ äëÿ íàñòðîéêè òîâàðîâ')) then
        save_log("Âû ïîêèíóëè ëàâêó", logfile)
if Lavkaaikib[0] then
...
SkupSellzxc[0] = false
    ...
    elseif (text:find('^%[Èíôîðìàöèÿ%] {FFFFFF}Âàøà ëàâêà áûëà çàêðûòà')) then
        save_log("Âû ïîòåðÿëè ëàâêó", logfile)
if Lavkaaikib[0] then
   CentralGlMenu[0] = false
end
if Lavkaaiki34b[0] then
SkupSellzxc[0] = false
    ...
    if text:find('^.+ êóïèë ó âàñ .+, âû ïîëó÷èëè %$%d+ îò ïðîäàæè %(êîìèññèÿ %d ïðîöåíò%(à%)%)') then
        ...
        save_log(name.." êóïèë ó âàñ "..product.." çà "..money.."$", logfile)
    elseif text:find('^Âû êóïèëè .+ ó èãðîêà .+ çà %$%d+') then
        local product, name, money = text:match('^Âû êóïèëè (.+) ó èãðîêà (.+) çà %$(%d+)')
        save_log(name.." ïðîäàë âàì "..product.." çà "..money.."$", logfile)
    end
end 
...
if imgui.Button(u8'Çàêðûòü', imgui.ImVec2(-1, 29)) then
LogMenu[0] = not LogMenu[0]
    ...
    imgui.Begin(u8'Central rrADD Vip Version', recMenu, imgui.WindowFlags.AlwaysAutoResize + imgui.WindowFlags.NoTitleBar)
         if imgui.Checkbox(u8'Àâòîðåêîííåêò ïðè "You banned from this server"', onBanned) then
            if not ini.cfg.AutoReconnect then
                onBanned[0] = not onBanned[0]
                msg("Àâòîðåêîííåêò âûêëþ÷åí. Ðåäàêòèðîâàíèå òàêæå îòêëþ÷åíî")
            else
                msg("Âû "..(onBanned[0] and "âêëþ÷èëè" or "âûêëþ÷èëè") .. " àâòîðåêîííåêò ïðè çàáàíåíîì ïîäêëþ÷åíèè")      
                ini.autoreconnect.onBanned = onBanned[0]
                cfg_save()
            end
        end
        if imgui.Checkbox(u8'Àâòîðåêîííåêò ïðè ïàðîëå ñåðâåðà', onPassword) then
            if not ini.cfg.AutoReconnect then
                onPassword[0] = not onPassword[0]
                msg("Àâòîðåêîííåêò âûêëþ÷åí. Ðåäàêòèðîâàíèå òàêæå îòêëþ÷åíî")
            else
                msg("Âû "..(onPassword[0] and "âêëþ÷èëè" or "âûêëþ÷èëè") .. " àâòîðåêîííåêò ïðè ïàðîëå ñåðâåðà")  
                ini.autoreconnect.onPassword = onPassword[0] 
                cfg_save()
            end
        end
        if imgui.Checkbox(u8'Àâòîðåêîííåêò ïðè çàêðûòèè ñîåäèíåíèÿ ñ ñåðâåðîì', onKicked) then
            if not ini.cfg.AutoReconnect then
                onKicked[0] = not onKicked[0]
                msg("Àâòîðåêîííåêò âûêëþ÷åí. Ðåäàêòèðîâàíèå òàêæå îòêëþ÷åíî")
            else
                msg("Âû "..(onKicked[0] and "âêëþ÷èëè" or "âûêëþ÷èëè") .. " àâòîðåêîííåêò ïðè çàêðûòèè ñîåäèíåíèÿ")          
                ini.autoreconnect.onKicked = onKicked[0]
                cfg_save()
            end
        end
        if imgui.Checkbox(u8'Àâòîðåêîííåêò ïðè çàíÿòîì/íåâåðíîì íèêå', onRejected) then
            if not ini.cfg.AutoReconnect then
                onRejected[0] = not onRejected[0]
                msg("Àâòîðåêîííåêò âûêëþ÷åí. Ðåäàêòèðîâàíèå òàêæå îòêëþ÷åíî")
            else
                msg("Âû "..(onRejected[0] and "âêëþ÷èëè" or "âûêëþ÷èëè") .. " àâòîðåêîííåêò ïðè çàíÿòîì/íåâåðíîì íèêå")
                ini.autoreconnect.onRejected = onRejected[0] 
                cfg_save()
            end
        end
        if imgui.SliderFloat(u8'Çàäåðæêà', Reconnect.delay, 0.0, 1000.0) then
...
recMenu[0] = not recMenu[0]
    ...
    imgui.Begin(u8'Cerrntral rrADD Vip Version', outhmenu, imgui.WindowFlags.AlwaysAutoResize + imgui.WindowFlags.NoTitleBar)
        ...
        if imgui.Button(u8"Î÷èñòèòü ïîëÿ") then
            msg("Ïîëÿ î÷èùåíû")
            imgui.StrCopy(NickName, '')
            imgui.StrCopy(Password, '')
        end
        if imgui.Combo(u8"Ìåñòî ñïàâíà", current_item_spawn, items_spawn, #item_spawn) then
...
if imgui.Button(faicons("SERVER") .. u8' Ëîã îïåðàöèé', imgui.ImVec2(200, 50)) then tab = 1 end
if imgui.Button(faicons("SHOP") .. u8' Ñêóï', imgui.ImVec2(200, 50)) then tab = 5 end
if imgui.Button(faicons("GEAR") .. u8' Íàñòðîéêè', imgui.ImVec2(200, 50)) then tab = 2 end
if imgui.Button(faicons("USER") .. u8' Èíôîðìàöèÿ', imgui.ImVec2(200, 50)) then tab = 4 end

    imgui.SetCursorPos(imgui.ImVec2(215, 28))
if imgui.BeginChild('Name', imgui.ImVec2(), true) then

if tab == 1 then 
if imgui.Button(u8' Âûáðàòü äåíü äëÿ ïðîñìîòðà ëîãà', imgui.ImVec2(-1, 29)) then
LogMenu[0] = not LogMenu[0]
end
if Param then

imgui.CenterText(u8'Ëîã çà: ' ..Param)
...
if imgui.Checkbox(u8'Àâòîðåêîííåêò', AutoReconnect) then     
        msg("Âû "..(AutoReconnect[0] and "âêëþ÷èëè" or "âûêëþ÷èëè") .. " àâòîðåêîííåêò") 
        if AutoReconnect[0] then msg("Ïîìíèòå, ÷òî íà íåêîòîðûõ ñåðâåðàõ çà ýòî âàñ ìîãóò çàáëîêèðîâàòü") end
        ini.cfg.AutoReconnect = AutoReconnect[0]
        cfg_save()
    end
imgui.SameLine()
if ini.cfg.AutoReconnect then 
    if imgui.Button(u8'Íàñòðîèòü àâòîðåêîííåêò', imgui.ImVec2(-1, 29)) then
    recMenu[0] = not recMenu[0]
    end
end
     if imgui.Checkbox(u8'Àâòîëîãèí', AutoLogin) then
         ini.config.AutoLogin = AutoLogin[0]
         cfg_save()
    end
if ini.config.AutoLogin then
    if imgui.Button(u8'Íàñòðîèòü àâòîëîãèí', imgui.ImVec2(-1, 29)) then
    outhmenu[0] = not outhmenu[0]
    end
end 
imgui.Separator()
if imgui.Combo(u8'Âûáîð òåìû', theme, new['const char*'][#themesList](themesList), #themesList) then 
    themes[theme[0]+1].func() 
    iniSave() 
end
imgui.Separator()
if imgui.Checkbox(u8' Îòêðûòèå ìåíþ "Central Helper" ïðè àðåíäå ëàâêè', Lavkaaikib) then
...
imgui.SameLine()
	if imgui.Button(u8" Ñîõðàíèòü") then
		...
		cfg_save()
		msg("Ñîõðàíåíî! Àêòèâàöèÿ - "..ini.cfg.activation, -1)
		script_reload()
	end
imgui.Separator()
if imgui.Checkbox(u8"Àâòîeäà", eat) then
    activated = not activated
end
if activated then
    if imgui.Combo(u8"Âûáåðèòå íóæíóþ åäó", eat_choice, items, #method) then
    ...
    if imgui.Button(u8'Îáíîâèòü(âîçìîæíî çàâèñàíèå èãðû íà 10-15 ñåêóíä)') then
...
imgui.CenterText(u8'Author: Theopka')
				if imgui.Button(faicons("BELL") .. u8" Ïåðåéòè â ÒÃÊ", imgui.ImVec2(-1, 25)) then openLink("https://t.me/TheopkaStudio") end
				imgui.Separator()
imgui.CenterText(u8' Êîìàíäû')
imgui.Text(u8'/cent - Ãë.Ìåíþ')
imgui.Text(u8'/recon - Ðåêîíåêò')
imgui.Text(u8'/calc - Êàëüêóëÿòîð')

elseif tab == 5 then
    if imgui.Button(faicons("play") .. u8" Íà÷àòü ñêóï ", imgui.ImVec2(-1, 30)) then
...
imgui.Text('')
    ...
    for i = 1, 100 do
        imgui.BeginChild('##child_' .. i, imgui.ImVec2(-1, 125), true)
            ...
            imgui.InputTextWithHint(u8'##se_' .. i, u8'Íàïðèìåð: Ïëàòèíîâàÿ ðóëåòêà', variables[i].se, 255)
            ini.cfg['se' .. i] = u8:decode(str(variables[i].se))
            imgui.InputTextWithHint(u8'##i_' .. i, u8'Íàïðèìåð: "1, 500000"', variables[i].i, 255)
            ini.cfg['i' .. i] = u8:decode(str(variables[i].i))
        imgui.EndChild()
    end
    cfg_save() -- Ñîõðàíåíèå çíà÷åíèé âûíåñåíî çà ïðåäåëû öèêëà
    imgui.EndChild()
end
    imgui.EndChild()
...
if not isSampfuncsLoaded() or not isSampLoaded() then return end
    ...
    msg('Ñêðèïò Çàãðóæåí!')
    msg('Àêòèâàöèÿ: /'..ini.cfg.activation)
    msg('Àâòîð Theopka')
            ...
            if(raknetBitStreamReadInt8(bs) == 9) then 
                ...
                if(show_type == 3) then
                    ...
                    if(level_type == 5) then
                        lua_thread.create(function()
                            wait(200)
                            msg("Âûáèðàåì ìåñòî ñïàâíà - "..u8:decode(item_spawn[ini.autologin.spawn_use+1]))                      
                    ...
                    if(level_type == 17) then
                        lua_thread.create(function()
                            wait(2000)
                            msg("Ïðîáóåì àâòîðèçîâàòüñÿ ïîä íèêîì - " .. ini.autologin.nickname)
...
lua_thread.create(function()
msg("Ñêðèïò áóäåò ïåðåçàãðóæåí ÷åðåç 2 ñåêóíäû!")
wait(2000)
msg("Ïåðåçàãðóçêà...")
thisScript():reload()
end)
...
lua_thread.create(function()
msg("Ñêðèïò áóäåò âûãðóæåí ÷åðåç 2 ñåêóíäû!")
wait(2000)
msg("Âûãðóçêà...")
thisScript():unload()
end)
    ...
    if type == 1 then
        lua_thread.create(function()
            if Reconnect.active then 
                msg("Ïîïûòêà ðåêîííåêòà áûëà îòìåíåíà ò.ê. óæå åñòü ñóùåñòâóþùàÿ")
                return
             end
...
function sampev.onAuthenticationRequest()
  msg("Ïûòàåìñÿ ïîäêëþ÷èòüñÿ ê ñåðâåðó.. Ïîïûòêà: " .. connects)
end

function sampev.onConnectionRequestAccepted()
  msg("Ïîäêëþ÷èëèñü. Âõîäèì â èãðó.")
end

function sampev.onConnectionAttemptFailed()
  msg("Ñåðâåð íå îòâå÷àåò.. Ïîïûòêà: " .. connects)
  connects = connects + 1
end

function sampev.onConnectionNoFreeSlot()
  msg("Ñåðâåð ïîëîí.. Ïîïûòêà: " .. connects)
  connects = connects + 1
end

function sampev.onConnectionBanned()
    msg("Âû çàáàíåíû íà äàííîì ñåðâåðå")
...
function sampev.onConnectionRejected()
    msg("Íåäîïóñòèìûé íèê èëè íèê çàíÿò.")
